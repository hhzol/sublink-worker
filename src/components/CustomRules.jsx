...
export const CustomRules = (props) => {
  ...
  // 将内联脚本的全局逻辑转化为标准的 JS 字符串，避免反引号大括号语法冲突
  const scriptContent = `
    function customRulesData() {
      return {
            ...
            if (this.mode === 'json') {
              try {
                ...
                if (Array.isArray(parsed)) {
                  ...
                  this.jsonError = ${JSON.stringify(t('mustBeArray'))};
                }
              } catch (e) {
        ...
        clearAll() {
          if (!confirm(${JSON.stringify(t('confirmClearAllRules'))})) {
            return;
          }
    ...
    <div x-data="customRulesData()" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="col-span-1 md:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center gap-1">
                    {t('customRuleSetUrl') || '规则集地址 (Rule Set URL)'}
                    <i class="fas fa-info-circle text-gray-400 hover:text-primary-500 cursor-help" title={t('customRuleSetUrlTooltip') || '远程 Rule-Set / Rule-Provider 的下载链接'}></i>
                  </label>
                  <input
                    type="text"
                    x-model="rule.rule_set_url"
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 font-mono text-sm"
                    placeholder={t('customRuleSetUrlPlaceholder') || 'https://example.com/ruleset.srs'}
                  />
                </div>
      ...
      <div x-show="mode === 'json'" {...{'x-transition:enter': 'transition ease-out duration-300', 'x-transition:enter-start': 'opacity-0 transform scale-95', 'x-transition:enter-end': 'opacity-100 transform scale-100'}}>
        <ValidatedTextarea
          ...
          model="jsonContent"
          placeholder='[{"name": "MyRule", "rule_set_url": "https://example.com/rules.srs", "outbound": "Proxy"}]'
          variant="mono"
